...
byte buffer_channel_arduino[number_of_channels] = {0x00, 0x00, 0x00, 0x00}; // buffer to store filetered DMX data
...
void setup()
{
  ...
  analogWrite(LEDStripWhiteControlPin, 0);
  analogWrite(LEDStripRedControlPin,   0);
  analogWrite(LEDStripGreenControlPin, 0);
  analogWrite(LEDStripBlueControlPin,  0);
  ...
  { // check size to avoid unneeded checks
    ...
    if (match_artnet == 1)
    {
      ...
      if (is_opcode_is_dmx)
      {
        ...
        Serial.print("White: ");
        Serial.println(buffer_channel_arduino[0], HEX);
        Serial.print("Red: ");
        Serial.println(buffer_channel_arduino[1], HEX);
        Serial.print("Green: ");
        Serial.println(buffer_channel_arduino[2], HEX);
        Serial.print("Blue: ");
        Serial.println(buffer_channel_arduino[3], HEX);
